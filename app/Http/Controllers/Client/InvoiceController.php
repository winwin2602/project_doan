...
class InvoiceController extends Controller
{
	...
	public function index()
	{
		if(Cookie::has('item')){
			...
			return view('client.invoice.cart',compact('cart','services'));
		}else{
			$cart = [];
			$services = Cost::all();
			return view('client.invoice.cart',compact('cart','services'));
	...
	public function delete($id)
	{
		...
		Cookie::queue(Cookie::make('item', serialize($cart), 60*24));
		return back()->with('message', trans('allclient.delete_success'));
	}
	...
	public function addinvoice($id, Request $request)
	{
		...
		if(Cookie::has('item')){
        	// Kiem tra room co trong don hang chua?
			$found = in_array($id, array_column(unserialize(Cookie::get('item')), 'id'));
			if($found){
				return back()->with('message','Room already exists');
			}
			$cart = unserialize(Cookie::get('item'));
		} 
		$cart[] = ['id' => $id, 'name' => $room->name, 'price' => $room->price, 'adult' => $room->adult, 'kid' => $room->kid, 'thumbnail' => $room->thumbnail, 'room_code' => $room->room_code, 'location' => $room->location, 'adult' => $room->adult, 'kid' => $room->kid, 'acreage' => $room->acreage];
		//Khởi tạo giỏ hàng bằng cookie
		Cookie::queue(Cookie::make('item', serialize($cart), 60*24));
		return redirect('invoice')->with('message',trans('allclient.add_invoice_success'));
	}

	// Kiểm tra giá
	public function check_price(Request $request){
		$cart = unserialize(Cookie::get('item'));
		// Tao Cookie luu lai thong tin checkout
		...
		if($request->service){
				...
				foreach ($request->service as $service) {
					$services->orWhere('id',$service);
				}
				$services = $services->distinct()->get();
				Session::put('cost_overrun', $services ); // List dịch vụ khách chọn
		}
		// Kiem tra ngay checkin phai nho hon ngay checkout
		if($checkin >= $checkout){
			return back()->with('message','Please check the date you chose');
		}
		if($cart){
			
			$price = Price::pluck('date','id')->where('isdeleted',0)->toArray();
		// Check trạng thái của room
			foreach ($cart as $value) {
				...
				foreach ($check as $value2) {
					if( (date('Y-m-d', $checkin) < $value2->booking->check_in_date && date('Y-m-d', $checkout) < $value2->booking->check_in_date) || (date('Y-m-d', $checkin) > $value2->booking->check_out_date && date('Y-m-d', $checkout) > $value2->booking->check_out_date)  ){
						...
						$request->session()->flash('message'.$value['id'], trans('allclient.available'));
						return back()->with('message',trans('allclient.available'));
					}
				}
			}
			for ($currentDate = $checkin; $currentDate <= $checkout;  
				$currentDate += (86400)) { 
		...
		foreach ($array as $key=> $value) {
			if(in_array($value, $price)){
				$special_day[] = $result[$key];
				$array[$key] = date('d-m-Y',strtotime($value)).' : '.($total_sv+$sum+($sum*Price::where('date',$result[$key])->value('percent')/100)).' VNĐ || ('.Price::where('date',$result[$key])->value('description').' : '.Price::where('date',$result[$key])->value('percent').'%)<br>';
				$tongtien+=($sum+($sum*Price::where('date',$result[$key])->value('percent')/100));
			}
			else{
				$array[$key] = date('d-m-Y',strtotime($value)).' : '.($total_sv+$sum).' VNĐ<br>';
				$tongtien+=$sum;
			}  
		}// end foreach

		// Tạo session chứa dữ liệu để tái sử dụng
		$total_amount = $tongtien+($total_sv*count($array)*count($cart));
		Session::put('total_amount',$total_amount );
		Session::put('adult',$adult );
		Session::put('kid',$kid );
		Session::put('special_day',$special_day );

		return view('client.invoice.checkout', compact('array','cart','tongtien','total_sv'));
	}
